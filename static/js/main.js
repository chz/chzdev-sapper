(function(a) {
  ...
  function b() {
    const d = a('.portfolio-grid');
    const f = a('.portfolio-filters');
    d && (d.shuffle({
      speed: 450,
      itemSelector: 'figure'
  ...
  function c() {
    const d = a(window).width();
    const f = a('#site_header');
    d < 992 ? (f.addClass('mobile-menu-hide'), setTimeout(function() {
      f.addClass('animate')
  ...
  a(function() {
    ...
    f.imagesLoaded(function() {
      ...
      a(this).val().length === 0 && a(this).parent('.form-group').removeClass('form-group-focus')
    }), a('#map').googleMap({
      zoom: 16
    }), a('#map').addMarker({
